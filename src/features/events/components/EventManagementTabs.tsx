...
const STATUS_TRANSITIONS: Record<
  string,
  { action: string; label: string; cls: string }[]
> = {
  DRAFT: [
    { action: 'publish', label: 'Publish', cls: 'bg-emerald-600 hover:bg-emerald-500' },
  ],
  PUBLISHED: [
...
export default function EventManagementTabs({ event, onEventUpdate }: Props) {
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(sanitiseErrorMessage('Failed to save changes.'));
    } finally {
      setEditSaving(false);
    }
  };
        ...
        <div className="mt-6 space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              {
                label: 'Revenue',
                value: event.revenue != null ? `$${event.revenue.toLocaleString()}` : '—',
              },
              {
                label: 'Scan Rate',
                value: event.scanRate != null ? `${event.scanRate}%` : '—',
              },
            ].map(({ label, value }) => (
              <div
                key={label}
                className="rounded-xl border border-[#4D21FF]/30 bg-[#0a0f24] p-4"
              >
      ...
      {activeTab === 'Edit' && (
        <form onSubmit={handleEditSave} className="mt-6 space-y-5">
          {[
            ...
            <div key={id}>
              <label className="mb-1 block text-sm font-medium text-white" htmlFor={id}>
                {label}
              </label>
              <input
                id={id}
        ...
        <div className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={refresh}
              className="text-xs text-[#21D4FF] hover:underline"
            >
              Refresh
            </button>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3 rounded-lg border border-yellow-700/40 bg-yellow-950/20 p-4">
            ...
            <p className="text-sm text-yellow-100/90">
              Are you sure you want to <strong>{confirmAction}</strong> &ldquo;
              {event.name}&rdquo;?
            </p>
          </div>
