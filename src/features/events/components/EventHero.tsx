...
export function EventHero({ event, onShare, shareCopied }: EventHeroProps) {
  ...
  const organizerId =
    event.organizerId ?? event.organizer?.name?.toLowerCase().replace(/\s+/g, '-');
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="-mt-16 relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-2">
            <span
              className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium ${statusStyle}`}
            >
              {event.status}
            </span>
