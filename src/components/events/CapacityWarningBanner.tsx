...
export function CapacityWarningBanner({ sold, capacity, eventTitle, eventId }: CapacityWarningBannerProps) {
  ...
  return (
    <div className={`p-4 mb-4 border rounded-lg ${bannerClasses}`}>
      <div className="flex items-center justify-between">
        <p>
          <span className="font-bold">⚠️ {eventTitle}</span> is {percentageSold}% sold ({remainingTickets} tickets remaining).
        </p>
        <div className="flex items-center">
          <Link href={`/events/manage/${eventId}/edit`}>
            <a className="px-4 py-2 mr-4 text-sm font-medium bg-white rounded-md hover:bg-gray-200">Increase Capacity</a>
          </Link>
          <button onClick={handleDismiss} className="text-lg font-bold">×</button>
        </div>
      </div>
    </div>
  );
