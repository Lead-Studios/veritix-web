  ...
  return (
    <li
      ...
      className={`flex flex-wrap items-center justify-between gap-4 rounded-xl border border-[#4D21FF]/30 bg-[#000625] px-5 py-4 transition-colors ${
        isSoldOut ? 'opacity-50 grayscale' : 'hover:border-[#4D21FF]/60'
      }`}
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-3">
          <h3 className="truncate text-base font-semibold text-white">{ticket.name}</h3>
          <InventoryStatusBadge sold={ticket.sold} total={ticket.total} />
        </div>
        {ticket.price && <p className="mt-1 text-sm text-[#21D4FF]/80">{ticket.price}</p>}
      </div>
