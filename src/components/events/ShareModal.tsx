...
export default function ShareModal({ url, title, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl p-6 w-80 flex flex-col gap-3"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="font-semibold text-lg">Share Event</h2>
        <button onClick={copy} className="w-full border rounded-lg py-2 text-sm">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="w-full border rounded-lg py-2 text-sm text-center block"
        >
          Share on Twitter
        </a>
        <a
