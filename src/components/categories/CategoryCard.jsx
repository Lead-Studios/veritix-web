...
const CategoryCard = ({ icon, title, description, count, events, id, isMobile }) => {
  // Responsive design - more compact for mobile horizontal scroll
  if (isMobile) {
    return (
      <div className="relative flex flex-col items-center justify-center p-4 rounded-xl bg-[#0a0e21] border border-[#1c2044] hover:border-blue-500 transition-all overflow-hidden group cursor-pointer h-44">
        <div className="absolute top-2 right-2 bg-blue-600 text-white text-xs px-2 py-1 rounded-full z-20">
          {count}+
        </div>
        
        <div className="text-blue-500 mb-2 text-3xl">
          {icon}
        </div>
        
        <h3 className="text-blue-500 font-bold text-base mb-1">{title}</h3>
        
        <p className="text-gray-300 text-xs text-center line-clamp-2">{description}</p>
