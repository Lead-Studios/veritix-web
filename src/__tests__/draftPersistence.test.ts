...
describe("draftPersistence", () => {
  ...
  it("writes the draft to localStorage before calling the API", async () => {
    ...
    const stored = JSON.parse(localStorage.getItem(DRAFT_STORAGE_KEY) as string) as EventDraft;
  ...
  it("returns the local draft when the API request fails", async () => {
    ...
    expect(JSON.parse(localStorage.getItem(DRAFT_STORAGE_KEY) as string).formData).toEqual(formData);
  });
