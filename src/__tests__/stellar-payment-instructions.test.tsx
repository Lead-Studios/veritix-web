...
describe('StellarPaymentInstructions', () => {
  ...
  it('renders Stellar payment details and copy controls', async () => {
    ...
    expect(
      screen.getByText('Required — your payment will fail without the memo'),
    ).toBeInTheDocument();
    expect(screen.getByText('25.5 XLM')).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /copy destination address/i }),
    ).toBeInTheDocument();
  ...
  it('redirects to tickets when polling returns PAID', async () => {
    ...
    await waitFor(() => {
      expect(toast.success).toHaveBeenCalledWith(
        '\uD83C\uDF89 Tickets issued! Check your email.',
      );
