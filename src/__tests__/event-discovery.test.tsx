...
describe('Event Discovery - Search and Filter', () => {
  ...
  it('reduces event list when a category filter is applied', async () => {
    await waitFor(() => screen.getAllByText(/Summer Dance Festival/i));
    const initialCount = screen.getAllByRole('link', {
      name: /Summer Dance Festival/i,
    }).length;
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole('button', { name: /^remove festival filter$/i }),
      ).toBeNull(),
    );
  ...
  it('handles combined search and category filter', async () => {
    ...
    const removeMusicButton = screen.getByRole('button', {
      name: /remove music filter/i,
    });
    fireEvent.click(removeMusicButton);

    await waitFor(() =>
      screen.getAllByText(/Summer Dance Festival|Electronic Music Night/i),
    );
    ...
    await waitFor(() =>
      expect(
        screen.getAllByRole('link', { name: /Summer Dance Festival/i }).length,
      ).toBeGreaterThan(0),
    );
    await waitFor(() =>
      expect(
        screen.queryAllByRole('link', { name: /Electronic Music Night/i }).length,
      ).toBe(0),
    );
  });
});
