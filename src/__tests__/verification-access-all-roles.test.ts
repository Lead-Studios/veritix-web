...
import {
  canAccessVerificationTools,
  getVerificationAccessDeniedMessage,
...
describe('verificationAccess role check permutations', () => {
  ...
  it('getVerificationAccessDeniedMessage handles null role', () => {
    expect(getVerificationAccessDeniedMessage(null)).toBe(
      'Please sign in to access verification tools.',
    );
  });

  it('getVerificationAccessDeniedMessage handles non-null denied role', () => {
    expect(getVerificationAccessDeniedMessage('attendee')).toBe(
      'Verification tools are restricted to authorized staff only.',
    );
  });
});
