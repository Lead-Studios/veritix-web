...
vi.mock('@/components/events/ui/ImageUpload', () => ({
  default: ({ onChange }: { onChange: (file: File | null) => void }) =>
    React.createElement(
      'button',
      {
        type: 'button',
        ...
        onClick: () =>
          onChange(new File(['fake-image-bytes'], 'cover.png', { type: 'image/png' })),
      },
      'Upload',
    ),
}));
...
describe('Event Creation – end-to-end flow', () => {
  it('walks every section with valid data, submits the FormData payload, and redirects to /events/manage', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Give your event a catchy title'), {
      target: { value: 'Stellar Summit 2026' },
    });
    fireEvent.change(screen.getByPlaceholderText('Describe your event in details'), {
      target: {
    ...
    // ----- Section 4: Tickets -----
    fireEvent.change(screen.getByPlaceholderText('e.g. VIP, General admission'), {
    ...
    fireEvent.change(
      screen.getByPlaceholderText('e.g. 0x5aAeb6053F3E94C9b9A09f33669435E7Ef1BeAed'),
