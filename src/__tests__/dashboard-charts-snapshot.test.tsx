...
describe("Dashboard Chart Snapshot Tests", () => {
  ...
  it("snapshots RevenueChart with minimal data", () => {
    const { container } = render(<RevenueChart data={[{ month: "Jan", revenue: 500 }]} />);
  ...
  it("snapshots PerformanceChart with minimal data", () => {
    const { container } = render(<PerformanceChart data={[{ month: "Jan", value: 80 }]} />);
    expect(container.firstChild).toMatchSnapshot();
  });
  ...
  it("snapshots TicketTypeChart with minimal data", async () => {
    const { container } = render(<TicketTypeChart data={[{ type: "VIP", count: 10, revenue: 1500 }]} />);
